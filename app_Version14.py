...
LOGIN_LOGO_PATH = "login_logo.png"   # 可选：登录页左侧图标(建议 128x128 PNG)，没有会使用 emoji
...
def render_login_page():
    ...
    st.markdown("""
    <style>
        body, .stApp { background: radial-gradient(circle at 30% 30%, #eef6ff 0%, #f5f8fb 55%, #ffffff 100%) !important; }
        /* 登录卡片外层居中宽度 */
        .login-wrapper { max-width: """ + str(LOGIN_CARD_WIDTH_PX) + """px; margin: 2.2rem auto 3rem auto; }
        .login-card {
            background: #ffffff;
        ...
        /* 输入框标签颜色微调 */
        .stTextInput label { font-weight:500; color:#334155 !important; }
        .login-btn button {
        ...
        .fake-note {
            margin-top:.55rem; color:#64748b; font-size:.8rem;
        }
    </style>
    ...
    with st.container():
        ...
        if logo_b64:
            st.markdown(
                ...
                <div class="login-head">
                    <div>
                        <img src="data:image/png;base64,{logo_b64}" style="width:70px;height:70px;border-radius:18px;box-shadow:0 4px 10px -2px rgba(0,0,0,.15);" />
                    </div>
        ...
        with st.form("login_form"):
            ...
            with c1:
                ...
                password = st.text_input("账号密码", key="login_password", type=pwd_type, placeholder="可留空直接登录")
            with c2:
                if st.form_submit_button("👁", help="显示/隐藏密码", use_container_width=True):
                    st.session_state.show_pwd = not st.session_state.show_pwd
                    st.experimental_rerun()

            st.markdown('<div class="fake-note">提示：此登录页仅作界面展示，不做真实校验，直接点击下方登录即可进入系统。</div>', unsafe_allow_html=True)
            login_clicked = st.form_submit_button("登录", use_container_width=False)
            if login_clicked:
                st.session_state.logged_in = True
                # 可记录简单信息
                st.session_state.current_user = account.strip() or "访客用户"
                st.experimental_rerun()
...
# 如果尚未登录，渲染登录页并停止后续主逻辑
if not st.session_state.get("logged_in", False):
    render_login_page()
    st.stop()

# =============== 已登录后进入原始主程序（以下是原文件内容，稍作封装） ===============
...
# 仅在“数据查询”页：把“操作说明”放到导航栏下面（侧边栏区域）
if page == PAGE_DATA:
    with st.sidebar:
...
if page == PAGE_CAPTURE:
    with st.sidebar:
        ...
        with st.expander("⚙️ 高级选项", expanded=False):
            st.markdown("**置信度阈值**")
            confidence = st.slider("置信度阈值", 0.70, 0.99, float(st.session_state.get("confidence", 0.90)), 0.01, format="%.2f")
            ...
            with st.expander("数据来源", expanded=False):
                source = st.radio("请选择来源", ["图片", "视频", "摄像头"], index=["图片","视频","摄像头"].index(st.session_state.get("source","图片")), horizontal=True)
...
if page == PAGE_WELCOME:
    ...
    try:
        ...
        df_display_total = pd.concat([df_display, pd.DataFrame([{"类别": "总数", "数量": int(round(total_val)), "占比(%)": 100.00}])], ignore_index=True)
