...
if page == PAGE_WELCOME:
    ...
    try:
        ...
        # 新增：趋势分析按钮和折线图（将 Excel 序列日期 -> 真正日期）
        st.markdown("---")
        with st.expander("📈 趋势分析", expanded=False):
            ...
            if len(time_row_idx) and len(sick_row_idx):
                time_row = df.loc[time_row_idx[0], 1:].dropna()
                sick_row = pd.to_numeric(df.loc[sick_row_idx[0], 1:], errors="coerce").dropna()
