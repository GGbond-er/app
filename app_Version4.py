...
<style>
    ...
    /* 问答助手页的小字提示（参考示意图，居中、红色、较小字号、略加粗） */
    .qa-tagline {
        text-align: center;
        color: #ff4d4f;
        font-size: 0.95rem;
...
# 顶部品牌区：tu_an 在左、wenzi 在右（每个页面顶部统一展示）
brand_c1, brand_c2 = st.columns([1, 6], gap="small")
with brand_c1:
    try:
        st.image(BRAND_LOGO_PATH, use_column_width=False, width=64)
    except Exception:
        st.info("缺少 tu_an.png，请将其放到应用根目录")
with brand_c2:
    try:
        st.image(BRAND_TEXT_PATH, use_column_width=False, width=220)
    except Exception:
        st.info("缺少 wenzi.png，请将其放到应用根目录")
