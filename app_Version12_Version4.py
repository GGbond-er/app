...
<style>
    ...
    /* —— 数据查询页：建议模块 —— */
    /* 旧的 <hr>（兼容），用更粗的上边框；并与内容左对齐 */
    .ai-hr {
        border: 0;
        height: 0;
        border-top: 6px solid #7c8aa0;   /* 加粗 */
        width: 100%;                      /* 占满容器宽度，与左侧对齐 */
        margin: 0.5rem 0 0.9rem 0;        /* 去掉居中 */
        border-radius: 2px;
    }
    /* 更可靠的粗线表现（首选），并与内容左对齐 */
    .ai-hr-line {
...
if page == PAGE_WELCOME:
    ...
    # 开始识别页：下移并居中显示，避免被顶部遮挡
    st.markdown('<div class="loading-hint" style="margin:5.5rem auto 0;text-align:center;font-size:1.1rem;color:#334155;">模型正在加载中......</div>', unsafe_allow_html=True)
