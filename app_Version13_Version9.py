...
<style>
    ...
    section[data-testid="stSidebar"] div[role="radiogroup"] label p { text-align: left !重要; margin: 0; }
...
if page == PAGE_DATA:
    with st.sidebar:
        with st.expander("📝 操作说明", expanded=False):
            # 使用 white-space: pre-line 保留行首空格和换行，精确保留“ 2.”与“ 3.”的前导空格
            st.markdown(
...
if page == PAGE_WELCOME:
    ...
    # 使用 video-2.mp4 替换原首页图片，并缩小显示以便完全呈现
    video_path = "video-2.mp4"
    if os.path.exists(video_path):
        # 中间列显示，整体宽度约为页面的 50%，以避免占满屏幕
    ...
    try:
        ...
        with st.expander("📈 趋势分析", expanded=False):
            ...
            if len(time_row_idx) and len(sick_row_idx):
                ...
                # 关键修复：使用 numpy 值，避免把 Excel 列名当作索引展示
