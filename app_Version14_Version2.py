...
def render_login_page():
    ...
    with st.container():
        ...
        if logo_b64:
            st.markdown(
                ...
                <div class="login-head">
                    <div><img src="data:image/png;base64,{logo_b64}" style="width:70px;height:70px;border-radius:18px;box-shadow:0 4px 10px -2px rgba(0,0,0,.15);" /></div>
        ...
        # 表单（移除眼睛按钮、移除提示文字、移除密码占位提示）
        with st.form("login_form"):
            account = st.text_input("账号名称", key="login_account", placeholder="例如：farmer_001")
            password = st.text_input("账号密码", key="login_password", type="password", placeholder="")  # 无提示语
