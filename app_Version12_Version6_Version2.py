...
<style>
    ...
    /* —— 交流论坛样式 —— */
    .forum-title { font-size: 1.6rem; font-weight: 800; color: #111827; margin: 0.25rem 0 0.75rem 0; }
    .forum-section-title { font-size: 1.25rem; font-weight: 800; color: #0f172a; margin: 0.5rem 0 0.5rem 0; display:flex; align-items:center; gap:.4rem; }
    .post-block { border: 1px solid #e5e7eb; border-radius: 12px; padding: 12px 14px; margin: 8px 0; background:#fff; }
...
<div>{p['prefix']} {p['title']}（热度值：{p['heat']:02d}）</div>
<div class="post-meta">作者：{p['author']} | 发布时间：{p['time']}</div>
    ...
    if "forum_posts" in st.session_state and st.session_state.forum_posts:
        for p in reversed(st.session_state.forum_posts[-3:]):  # 最近三条
            st.markdown(
                f"""<div class="post-block">
<div>🆕 {p['title']}（热度值：{p.get('heat',0):02d}）</div>
<div class="post-meta">作者：{p.get('author','匿名')} | 发布时间：{p.get('created_at','')}</div>
    ...
    for p in latest_posts:
        st.markdown(
            f"""<div class="post-block">
<div>{p['title']}（热度值：{p['heat']:02d}）</div>
<div class="post-meta">作者：{p['author']} | 发布时间：{p['time']}</div>
</div>""",
            unsafe_allow_html=True
        )
