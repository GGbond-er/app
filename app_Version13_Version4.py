...
# 自定义CSS样式（含本次修改：放大标题和正文、专家回复加粗、侧边栏单选左对齐、开始检测按钮样式）
st.markdown("""
<style>
...
if page == PAGE_WELCOME:
    ...
    # 将提示语替换为“开始检测”按钮，位置保持居中不变
    st.markdown(
        '<div class="loading-hint" style="margin:2rem 0 0; text-align:center;">'
        '<button class="detect-btn">开始检测</button>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    # 顶部（标题下方）：操作说明按钮（移动到这里）
    instr_col, _ = st.columns([1, 4])
    with instr_col:
        with st.expander("📝 操作说明", expanded=False):
            st.markdown(
    ...
    try:
        ...
        sum_row = pd.DataFrame([{
            "类别": "总数",
            "数量": int(round(total_val)),
            "占比(%)": 100.00
        }])
        df_display_total = pd.concat([df_display, sum_row], ignore_index=True)
        ...
        with st.expander("📈 趋势分析", expanded=False):
            ...
            if len(time_row_idx) and len(sick_row_idx):
                ...
                # 关键修复：使用 numpy 值，避免把 Excel 列名当作索引展示出来
                ...
                if len(trend_df) == 0:
                    ...
                    if np.issubdtype(show_df["时间"].dtype, np.datetime64):
                        show_df["时间"] = show_df["时间"].astype("datetime64[ns]").astype("datetime64[ms]")
                    ...
                    try:
                        import plotly.express as px
                        fig = px.line(
                            trend_df, x="时间", y="患病鱼总数",
                            title="患病鱼总数随时间变化趋势",
                            labels={"时间": "时间", "患病鱼总数": "患病鱼总数"}
                        )
...
恭喜您及时检测到这些疾病！通过积极治疗和预防，您的鱼群有望恢复健康。如有不确定之处，建议寻求专业指导。保持细心照料，水族生活更安心！
    ...
    if st.session_state.get("forum_view") == "detail_post1":
        ...
        # 详情页内容（本次：标题更大、正文更大，"李潮副研究员回复："加粗）
