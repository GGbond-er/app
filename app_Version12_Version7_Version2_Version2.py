...
<style>
    ...
    .read-btn {
        ...
        padding: 4px 10px; border-radius: 8px; font-size: .9rem;
    }
...
3) 寄生虫性烂鳃病的防治，先用 0.5 mg/L 晶体敌百虫全池泼洒，杀灭寄生虫。隔两天，再用 0.3 mg/L 强氯精全池泼洒灭菌，以防寄生虫叮咬的伤口继发感染。
        ...
        for i, p in enumerate(hot_posts):
            ...
            with card:
                # 标题行：左标题 + 右按钮（紧邻标题）
                tcol, bcol = st.columns([0.85, 0.15], vertical_alignment="center")
                with tcol:
                ...
                with bcol:
                    if st.button("阅读全文", key=f"hot_read_{i}", help="查看此帖详情", type="secondary", use_container_width=True):
                        if i == 0:
                            st.session_state["forum_view"] = "detail_post1"   # 仅第一篇提供详情
                            # 不调用 rerun；按钮点击会自动重跑
                        else:
                            st.info("该帖详情页即将上线～")
                # 元信息与热度条
