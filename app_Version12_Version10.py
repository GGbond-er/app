[file name]: app_Version12_Version9.py
[file content begin]
...
# 自定义CSS样式（含本次修改：放大标题和正文、专家回复加粗、侧边栏单选左对齐）
st.markdown("""
<style>
...
if page == PAGE_WELCOME:
    ...
    try:
        ...
        with st.expander("📈 趋势分析", expanded=False):
            ...
            # 从数据中提取时间序列和患病总数
            time_data = df.iloc[2, 1:].dropna().values  # 时间数据
            sick_data = df.iloc[3, 1:].dropna().values  # 患病总数数据
            
            # 确保数据长度一致
            min_len = min(len(time_data), len(sick_data))
            time_data = time_data[:min_len]
            sick_data = sick_data[:min_len]
            ...
            try:
                ...
                st.markdown("**趋势分析要点：**")
                if len(sick_data) > 1:
                    max_sick = max(sick_data)
                    min_sick = min(sick_data)
                    avg_sick = np.mean(sick_data)
                    ...
                    # 判断趋势
                    if sick_data[-1] > sick_data[0]:
                        st.write("- 📈 总体呈上升趋势，需加强疾病防控")
                    elif sick_data[-1] < sick_data[0]:
